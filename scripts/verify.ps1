...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-delta-gfx-shader-detail.ps1

dart tests/test_domain_review.dart

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-delta-gfx-shader-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-delta-gfx-shader-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-delta_gfx_shader_lab-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
